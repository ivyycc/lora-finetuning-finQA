...
# -*- coding: utf-8 -*-
"""
LoRA training for FinQA program synthesis — patched to avoid collator tensorization errors.

Key fixes vs v1:
- Custom data collator that strips non-tensor fields ('id', etc.) before padding.
- TrainingArguments(remove_unused_columns=True) so Trainer drops columns not used by the model forward.
"""
...
class CleanLMDataCollator(DataCollatorForLanguageModeling):
    def torch_call(self, features):
        # Drop non-tensor features like 'id' that break tokenizer.pad
...
def main():
    ...
    wandb.init(project="Research Project", name="finqa-lora", reinit=True)
    ...
    if args.bnb_4bit:
        try:
            ...
            print("bitsandbytes not available; running in full precision.", e)
    ...
    training_args = TrainingArguments(
        ...
        report_to=["wandb"],
        # IMPORTANT: let Trainer drop columns unused by the model's forward pass
    ...
    if not args.run_tag:
        ...
        args.run_tag = f"{base_name}_dora_e{args.epochs}_lr{args.lr}_{stamp}"  # <-- FIX: use args.model_name
