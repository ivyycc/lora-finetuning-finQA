...
# -*- coding: utf-8 -*-
"""
Few-shot Finetune for FinQA — remote checkpoints to HF each epoch + final upload.
...
from transformers import (
    AutoTokenizer, LlamaForCausalLM, Trainer,
    TrainingArguments, DataCollatorForLanguageModeling,
    EarlyStoppingCallback
)

from huggingface_hub import HfApi, login
from save_to_HF import PushToHubOnSave  # callback that uploads checkpoint on save
...
def main():
    ...
    ap.add_argument("--fewshot_n", type=int, default=200)
    ap.add_argument("--fewshot_eval_n", type=int, default=883)
    ...
    # HF remote checkpointing / final upload
    ...
    wandb.init(project="Research Project", name="fewshot-finetune", reinit=True)
    ...
    # Few-shot slices
    train_full = load_from_disk(os.path.join(args.data_dir, "train"))
    dev_full   = load_from_disk(os.path.join(args.data_dir, "dev"))
    train_ds = train_full.select(range(min(args.fewshot_n, len(train_full))))
    dev_ds   = dev_full.select(range(min(args.fewshot_eval_n, len(dev_full))))
    ...
    if not args.run_tag:
        ...
        args.run_tag = f"{base_name}_fewshot{len(train_ds)}_e{args.epochs}_lr{args.lr}_{stamp}"

    # HF auth + ensure repo exists
    ...
    training_args = TrainingArguments(
        output_dir="./_work_tmp",                 # scratch; locals deleted after upload
        ...
        warmup_ratio=0.06,
        weight_decay=0.0,                         # usually low/no WD for tiny few-shot sets
        max_grad_norm=1.0,                        # gradient clipping
        logging_steps=25,
        eval_strategy="epoch",
        save_strategy="epoch",                    # triggers PushToHubOnSave
        save_total_limit=None,                    # upload then delete
