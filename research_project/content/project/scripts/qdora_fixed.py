...
# -*- coding: utf-8 -*-
"""
QDoRA Finetune for FinQA — robust Trainer pattern.
...
from save_to_HF import PushToHubOnSave     # assumes you have the callback from earlier
...
def main():
    ...
    wandb.init(project="Research Project", name="qdora-1B-finetune", reinit=True)

    tokenizer = AutoTokenizer.from_pretrained(args.model_name, use_fast=True)
    if tokenizer.pad_token_id is None:
        tokenizer.pad_token = tokenizer.eos_token

    print("Loading QDoRA model...")
    
    quant_config = BitsAndBytesConfig(
        load_in_4bit=True,
        bnb_4bit_compute_dtype=torch.bfloat16 if (torch.cuda.is_available() and torch.cuda.get_device_capability(0)[0] >= 8) else torch.float32,
    ...
    # QLoRA best practice: prep model for k-bit training (casts norms to fp32, enables input grads, etc.)
    base_model = prepare_model_for_kbit_training(base_model)
    base_model.config.use_cache = False  # avoid checkpoint interference

    lora_cfg = LoraConfig(
        task_type=TaskType.CAUSAL_LM,
        r=16, lora_alpha=32, lora_dropout=0.05,
        use_dora=True,                      # DoRA: weight decomposition (mag + LoRA direction)
    ...
    training_args = TrainingArguments(
        ...
        # keep alias for your env:
        eval_strategy="epoch",
        save_strategy="epoch",

        # dtype gating: bf16 only on Ampere+; otherwise use your --fp16 flag
        bf16=(torch.cuda.is_available() and torch.cuda.get_device_capability(0)[0] >= 8),
        fp16=args.fp16 and not (torch.cuda.is_available() and torch.cuda.get_device_capability(0)[0] >= 8),
    ...
    if not args.run_tag:
        ...
        args.run_tag = f"{base_name}_qdora_e{args.epochs}_lr{args.lr}_{stamp}"  # <-- FIX: use args.model_name
